...
namespace NutriPlat.Api.Services // <-- Espacio de nombres importante
{
    public class WorkoutRoutineService : IWorkoutRoutineService
    {
        ...
        public async Task<UserWorkoutRoutineDto?> AssignRoutineToClientAsync(AssignWorkoutRoutineDto assignmentDto, string trainerId)
        {
            ...
            {
                _logger.LogInformation("Intento de reasignar rutina {WorkoutRoutineId} a cliente {ClientId} que ya está asignada.", assignmentDto.WorkoutRoutineId, assignmentDto.ClientId);
                return MapearAsignacionRutinaEntidadADto(existingAssignment, workoutRoutine, client, trainer);
            }
            ...
            try { await _context.SaveChangesAsync(); }
            catch (DbUpdateException ex) { _logger.LogError(ex, "Error al guardar la asignación de la rutina {WorkoutRoutineId} al cliente {ClientId}.", assignmentDto.WorkoutRoutineId, assignmentDto.ClientId); return null; }
            return MapearAsignacionRutinaEntidadADto(newAssignment, workoutRoutine, client, trainer);
        }
        ...
        public async Task<bool> UnassignRoutineFromClientAsync(string clientId, string workoutRoutineId, string requestingUserId)
        {
            ...
            var requester = await _userManager.FindByIdAsync(requestingUserId);
            if (requester == null || (assignment.AssignedByTrainerId != requestingUserId && requester.AppRole != UserRole.Admin))
            {
